...
start=$SECONDS
node --input-type=module -e "import { selectCases } from './.maestro/cloud-fakeship/select-cases.mjs'; selectCases(process.env.PROOF_CASES);"
...
PROOF_CONTROLLER_URL=http://127.0.0.1:35453 NODE_OPTIONS=--conditions=tlon-source pnpm --filter @tloncorp/tlon-bot-e2e exec tsx "$PWD/.maestro/cloud-fakeship/maestro-peer.ts" > "$PROOF_OUTPUT/peer.log" 2>&1 &
...
import { selectCases } from './.maestro/cloud-fakeship/select-cases.mjs';
const selected = selectCases(process.env.PROOF_CASES);
