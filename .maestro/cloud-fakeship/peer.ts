...
const actor = (name: string) => new TlonActorClient({
  shipName: name, shipUrl: name === '~zod' ? process.env.PROOF_PUBLIC_URL! : manifest[name].url, code: manifest[name].code,
});
const zod = actor('~zod');
const ten = actor('~ten');
...
async function main() {
  ...
  const evidence = { source: process.env.GITHUB_SHA, snapshotSource: snapshot?.source, previousFixtureCleared: snapshot ? true : null, controllerStreamMs, deskHashes: hashes, group, setupMs: Date.now() - started };
  ...
  await until('peer acknowledgement reaches zod', async () => (await zod.state.channelPosts(group.chatChannel)).some(p => p.authorId === '~ten' && p.text === `${tag} reply received`));
  writeFileSync(`${out}/peer-result.json`, JSON.stringify({ ...evidence, replyVerified: true, acknowledgementAt: new Date().toISOString(), elapsedMs: Date.now() - started }, null, 2));
