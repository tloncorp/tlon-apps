...
function Root({
  ...
  function shouldDrag(el: EventTarget, isDraggingDown: boolean) {
    ...
    if (
      ...
      date.getTime() - lastTimeDragPrevented.current.getTime() < 1000
    ) {
      lastTimeDragPrevented.current = new Date();
      return false;
    }
  ...
  function onMove(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (isDragging) {
      const draggedDistance =
        ...
        (event as unknown as React.PointerEvent<HTMLDivElement>).clientY;
  ...
  useEffect(() => {
    ...
    function onVisualViewportChange() {
      ...
      const diffFromInitial =
        initialViewportHeight.current - visualViewportHeight;
      const drawerHeight =
        drawerRef.current?.getBoundingClientRect().height || 0;
  ...
  function closeDrawer() {
    ...
    if (drawerRef.current) {
      const swipeAmount = getComputedStyle(drawerRef.current)
  ...
  function resetDrawer() {
    ...
    const currentSwipeAmount = Number(
      getComputedStyle(drawerRef.current!)
        .getPropertyValue('--swipe-amount')
        .slice(0, -2)
    );
...
>(({ children, onOpenAutoFocus, ...rest }, ref) => {
  const { drawerRef, onPress, onRelease, onAnimationStart, onMove } =
    useDrawerContext();
