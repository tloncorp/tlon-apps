::  fetch: retrieve metadata and raw responses from third-party URLs
::  temporary compatibility layer: %metagrab and %dumb-proxy remain installed.
::
::    serves metadata at /apps/groups/~/fetch/meta,
...
::
::    raw responses are served separately at /apps/groups/~/fetch/raw.
::    this is NOT the signed-request path for our own services.
::
...
++  raw-fetch
  ...
  =.  header-list.request
    ...
    ::NOTE  do not include the originating IP address
    %+  rap  3
    :~  'for="tm-fetch";'
        'proto='  ?:(secure 'https' 'http')
    ==
  [%pass /fetch/raw/[for]/(scot %t url.request) %arvo %i %request request *outbound-config:iris]
::
++  extract-data
  |=  $:  url=@t
...
++  on-init
  ^-  (quip card _this)
  :_  this
  [%pass /eyre/bind %arvo %e %connect [~ /apps/groups/~/fetch] dap.bowl]~
::
++  on-load
  |=  ole=vase
  |^  ^-  (quip card _this)
...
++  on-poke
  ...
  ::
    ...
    ?:  ?=([%apps %groups %~.~ %fetch %raw *] site)
      =/  rawpath  t.t.t.t.t.site
      ?~  rawpath
        [(spout:hutils id [404 ~] `(as-octs:mimes:html 'bad path')) this]
      ?~  t.rawpath
        [(spout:hutils id [404 ~] `(as-octs:mimes:html 'bad path')) this]
      ?~  rawurl=(slaw %uw i.rawpath)
        [(spout:hutils id [400 ~] `(as-octs:mimes:html 'target not @uw')) this]
      ?~  (de-purl:html u.rawurl)
        [(spout:hutils id [400 ~] `(as-octs:mimes:html 'target not parseable')) this]
      =.  url.request  u.rawurl
      =.  header-list.request
        %+  skip  header-list.request
        |=([key=@t @t] =('cookie' key))
      [[(raw-fetch [id secure] request)]~ this]
    ?.  ?=([%apps %groups %~.~ %fetch %meta *] site)
      :_  this
      %^  spout:hutils  id
        [404 ~]
      `(as-octs:mimes:html (cat 3 'bad route into ' dap.bowl))
    =/  site  t.t.t.t.t.site  ::  tmi
    ?+  site
      [(spout:hutils id [404 ~] `(as-octs:mimes:html 'bad path')) this]
    ::
        [@ ~]
