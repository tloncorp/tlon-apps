...
++  ca-core
  ...
  ++  ca-activity
    ...
    ++  on-post-delete
      ...
      =*  post  +<
      ^+  ca-core
      =/  =message-key    [[author id] id]
      =/  =source         [%channel nest group.perm.perm.channel]
      =/  thread=^source  [%thread message-key nest group.perm.perm.channel]
      =/  mention=?       (was-mentioned:utils content our.bowl)
      =;  action
        ::  don't forget to mark any thread replies as read too
        ::
        (send action [%read thread [%all ~ &]] ~)
      :+  %read  source
      ::NOTE  important that this matches the event generated by +on-post
      [%event %post message-key nest group.perm.perm.channel content mention]
    ::
    ++  on-reply-delete
      |=  [parent=v-post:c v-reply:c]
      ^+  ca-core
      =/  parent-key  [[author id] id]:parent
      ...
      ^-  action
      :*  %read   source
          ::NOTE  important that this matches the event generated by +on-reply
          %event  %reply
          [[author id] id]
          parent-key
          nest
  ...
  ++  ca-u-reply
    ...
    ?:  ?=(%set -.u-reply)
      ...
      ::
      ?~  reply.u-reply
        =.  ca-core  (on-reply-delete:ca-activity post u.u.reply)
        (put-reply ~ %set ~)
      ::
      =*  old  u.u.reply
      =*  new  u.reply.u-reply
