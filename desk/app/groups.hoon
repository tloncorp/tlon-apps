  ...
  ?+    mark  ~|(bad-mark+mark !!)
    ...
    ::
      ...
      ?:  ?=(%create -.c-groups)
        ...
        ?<  (~(has by groups) flag)
        =/  se-core  (se-init-req:se-core request-id)
      ...
      =/  =flag:g
        ...
        ==
      =/  se-core
        (se-init-req:(se-abed:se-core flag) request-id)
    ...
    ::
      ...
      =/  =flag:g
        ?-  -.a-groups
          %group   flag.a-groups
          %invite  flag.a-groups
          %leave   flag.a-groups
        ==
      =/  group-core
        (go-init-req:(go-abed:go-core flag) request-id)
      ?-    -.a-groups
          %group
    ...
    ::
        %group-action-4
      =+  !<(=a-groups:v8:gv vase)
      $(+< group-action-5+!>(`action:g`[0v0 a-groups]))
        ::  deprecated
        %group-action-3
      ?>  from-self
      =+  !<(=action:v2:gv vase)
      ...
      ?:  ?=([%cordon %shut *] diff)
        ...
        ?-  -.cordon-diff
          ...
          ::
              %pending
            =/  =action:v10:gv
              [0v0 %group flag %entry %pending q.cordon-diff %add ~]
            $(+< group-action-5+!>(action))
          ==
        ::
            %del-ships
          ?-    p.cordon-diff
            ...
            ::
            =/  =action:v10:gv
              [0v0 %group flag %entry %ask q.cordon-diff %deny]
            $(+< group-action-5+!>(action))
          ::
              %pending
            =/  =action:v10:gv
              [0v0 %group flag %entry %pending q.cordon-diff %del ~]
            $(+< group-action-5+!>(action))
          ==
        ==
      ...
      %+  roll  a-group-list
      |=  [=a-group:v8:gv =_cor]
      =/  =action:v10:gv  [0v0 %group flag a-group]
      ^$(+< group-action-5+!>(action))
    ::
        ::  deprecated
        %group-leave
    ...
    ::
      ...
      =/  =a-invite:v8:gv  [~ ~]
      =/  =action:v10:gv
        [0v0 %invite p.invite-0 (sy q.invite-0 ~) a-invite]
      $(+< group-action-5+!>(action))
    ::
        %invite-decline
      =+  !<(=flag:g vase)
  ...
  ::
    ...
    ?+  site  (emil not-found)
      ...
      ?+  method.request  (emil not-found)
          %'GET'
        ?>  ?=([%request @ ~] site)
        =/  =request-id:v10:gv  (slav %uv i.t.site)
...
++  reset-channel-perms
  ...
  =.  cards.core
    :_
      ...
      =/  =command:g
        [0v0 [%group flag [%channel nest %del-readers readers.channel]]]
      =/  cage  group-command+!>(command)
...
++  watch
  ...
  ::
    ...
    ?:  (se-is-banned:se-core src.bowl)
      ~|(%se-is-banned !!)
    se-abet:(se-watch:(se-abed:se-core [our.bowl name.pole]) [%request rest.pole])
  ::
  ...
  ::
      [ver=?(%v0 %v1) %groups ship=@ name=@ rest=*]
    ?>  from-self
    =/  ship=@p  (slav %p ship.pole)
    go-abet:(go-watch:(go-abed:go-core ship name.pole) ver.pole rest.pole)
  ::
...
++  arvo
  ...
  ::
    ...
    ?~  request=(~(get by requests) id)
      cor
    =/  result=response-body:v10:gv  [%pending poke-status.u.request]
    =.  requests
      (~(put by requests) id u.request(result `result))
    =/  =response:v10:gv  [id result]
    =.  cor
...
++  give-http-response
  |=  [id=@ta =response:v10:gv]
  =/  =json  (response:v10:enjs:gj response)
  =/  body  (as-octs:mimes:html (en:json:html json))
  %-  emil
  %+  spout:hutils  id
  [[200 ['content-type' 'application/json']~] `body]
++  se-core
  |_  [=flag:g =log:g =group:g =request-id:v10:gv gone=_|]
  ::
  +*  ad  admissions.group
  ::
  ++  se-core  .
  ...
  ++  give  |=(=gift:agent:gall (emit %give gift))
  ++  se-init-req
    |=  id=request-id:v10:gv
    se-core(request-id id)
  ::  +se-abed: init
  ::
  ++  se-abed
    |=  =flag:g
  ...
  ++  se-admin-subscription-paths
    ~>  %spin.['se-admin-subscription-paths']
    ^-  (list path)
    %+  skim  ~(tap in (~(gas in *(set path)) (turn ~(val by sup.bowl) tail)))
    |=  =path
...
::
++  go-core
  |_  [=flag:g =net:g =group:g =request-id:v10:gv gone=_|]
  +*  ad  admissions.group
  ::
  ++  go-core  .
  ...
  ++  go-watch
    ...
    ?+    pole  ~|(go-bad-watch+pole !!)
        [%request id=@ ~]
      ::  you can only watch your own requests
      ?>  from-self
  ...
  ++  go-agent
    ...
    ::
      ...
      ::
          %watch-ack
        ?~  p.sign  go-core
        =.  cor  (fail:l %watch-ack 'failed request watch' u.p.sign)
        go-core
      ::
          %fact
        =+  !<(=response-update:v10:gv q.cage.sign)
        =/  id=request-id:v10:gv  (slav %uv i.t.wire)
        ::  for now these are the same, but in the future we'd need to convert here
        =/  =response:v10:gv  response-update
