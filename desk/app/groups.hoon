...
++  submit-activity
  ...
  ?.  .^(? %gu /(scot %p our.bowl)/activity/(scot %da now.bowl)/$)
    cor
  %-  emit
  =/  =rail  activity-action-1+`action:v9:av`action
...
++  poke
  ...
  ::
    ...
    %-  emil
    :~  [%give %fact ~[pax] [%unsafe %http-response-header !>(response-header.pay)]]
        [%give %fact ~[pax] [%unsafe %http-response-data !>(data.pay)]]
        [%give %kick ~[pax] ~]
    ==
  ==
::
...
++  load
  ...
  ++  state-11-to-12
    ...
    %=  state-11
      -  %12
      groups  (~(run by groups.state-11) v11:net-group:v9:gc)
    ==
  --
::
++  inflate-io
...
++  peek
  ...
  ::
       [%x ver=?(%v0 %v1 %v2 %v3) %groups ~]
    =/  groups-11=groups:v11:gv  (~(run by groups) tail)
    =*  groups-9  (~(run by groups-11) v9:group:v11:gc)
    ?-    ver.pole
...
++  update-active-channel
  ...
  =/  r-groups-10=r-groups:v10:gv  [flag [%active-channel nest joined]]
  =.  cor  (give %fact ~[/v2/groups] group-response-2+r-groups-10)
  =/  r-groups-11=r-groups:v11:gv  [flag [%active-channel nest joined]]
  (give %fact ~[/v3/groups] group-response-3+r-groups-11)
::
++  take-channels
  |=  =sign:agent:gall
  ~>  %spin.['take-channels']
 ...
 ++  se-is-banned
  ...
  ++  se-c-group
    ...
    ::
        %blob
      ?>  se-src-is-admin
      ?>  (lte (met 3 (jam blob.c-group)) size-limit)
      ?:  =(blob.group blob.c-group)  se-core
      =.  blob.group  blob.c-group
      (se-update %blob blob.group)
    ::
        %entry
      ?>  se-src-is-admin
  ...
  ++  se-c-entry-ask
    ...
    ?-    c-ask
      ...
      =.  se-core
        ...
        =/  =rail  group-token+tok
        =/  =path  (weld se-area /ask/(scot %p ship))
        =.  se-core
          (give:se-core %fact ~[path] rail)
...
++  go-core
  ...
  ++  go-pass
    ...
    ++  go-wake-members
      ...
      =/  =rail  noun+[%group-wake flag]
      [%pass wire %agent [who dap.bowl] %poke rail]
    --
  ...
  ++  go-send-command
    ...
    =/  =rail  group-command+`c-groups:g`[%group flag c-group]
    (emit %pass wire %agent [p.flag server] %poke rail)
  ...
  ++  go-u-meta
    ...
    =.  meta.group  meta
    go-core
  ::  +go-u-blob: apply custom payload update
  ::
  ++  go-u-blob
    |=  blob=(unit @t)
    ~>  %spin.['go-u-blob']
    ^+  go-core
    =.  go-core  (go-response [%blob blob])
    ?:  go-our-host  go-core
    ::
    =.  blob.group  blob
    go-core
  ...
  ++  go-response
    ...
    =/  v3-paths  ~[/v3/groups [%v3 go-area]]
    =.  cor  (give %fact v3-paths group-response-3+r-groups-11)
    ...
    ?~  r-group-9  go-core
    =/  r-groups-9=r-groups:v9:gv  [flag u.r-group-9]
