...
::
|%
::  +expect-eq: checks for equality and pretty-prints the result
::
++  expect-eq
  ...
  =?  result  !(~(nest ut p.actual) | p.expected)
    ...
    ==  ==
  result
::  +expect-not-eq: checks for inequality and pretty-prints the result
::
++  expect-not-eq
  |=  [expected=vase actual=vase]
  ^-  tang
  ?.  ?&  =(q.expected q.actual)
      ...
      ==
    :~  [%palm [": " ~ ~ ~] [leaf+"not expected" (sell expected) ~]]
        [%palm [": " ~ ~ ~] [leaf+"actual  " (sell actual) ~]]
    ==
  ~
