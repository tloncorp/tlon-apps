...
++  v10
  ...
  +$  event
    $:  incoming-event
        ...
        child=?
    ==
  ::  $incoming-event: add %note-create and %note-edit variants
  ::
  +$  incoming-event
    $%  [%post post-event]
        [%reply reply-event]
  ...
  +$  note-event
    $:  id=@ud
        ...
        author=ship
    ==
  ::  $source: add %notebook and %note variants
  ::
  +$  source
    $%  [%base ~]
        [%group =flag:gv]
  ...
  +$  activity-bundle
    ...
    ==
  ::  $event-type: add %note-create and %note-edit
  ::
  +$  event-type
    $?  %chan-init
        %post
  ...
  ::  $default-volumes: add %note-create and %note-edit
  ::
  ++  default-volumes
    ^~
    ^-  (map event-type volume)
