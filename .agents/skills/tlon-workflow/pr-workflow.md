...
Always pass `--draft` to `gh pr create` unless the user has explicitly asked for a ready-for-review PR (e.g. "open it ready", "non-draft", "request review", or similar). This applies even when the change feels small or obviously correct — the user marks it ready themselves once they have looked it over.

## Template Discovery

Always check for and follow the PR template. Look for templates in this order:

1. `.github/PULL_REQUEST_TEMPLATE.md` in the current repository
2. `.github/pull_request_template.md` in the current repository
3. Organization-level template via `gh api repos/{org}/.github/contents/PULL_REQUEST_TEMPLATE.md`

To fetch org-level template, extract the org from the remote URL and query the `.github` repo.

If a template exists:
- Follow it exactly. Use the template's own section headings — do not rename them, drop any, reorder them, or substitute your own preferred structure (e.g. this guide's default `Description`/`Solution`/`Test plan`) over the sections the template defines. Every section the template ships with must appear in your body, including convention-driven ones like a `## Changelog:` entry that the repo expects on every PR.
- If the template includes fields you cannot fill (e.g., ticket/issue links), ask the user before creating the PR. Do not leave required fields empty or use placeholders.
- If this guide conflicts with the template, the template wins.

## PR Titles

Match the repo's existing title convention — check recent merged **PR titles** specifically (`gh pr list --state merged --limit 10`), not the commit log: squash-merge repos often have conventional-commit *commits* but plain *PR titles*, and inferring from commits leads to a `feat(scope):` prefix on a repo whose PRs are plain sentences. A `fix:`/`feat:` prefix only where merged PRs actually carry one, a `[ANDROID]`-style tag where that's the pattern, plain sentence case otherwise. Name the user-facing effect, not the implementation detail ("fix: voice memo recorder missing left padding", not "apply paddingStart in recording mode"). Keep it one line, specific, and accurate to the final diff — retitle if the scope changes after review.
...
```md
## Description
<context + problem; state facts directly>

## Solution
<intent and reasoning, not a line-by-line diff walkthrough>
...
<only when there were meaningful trade-offs>
```
