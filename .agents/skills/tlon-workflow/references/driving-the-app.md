...
On an EAS Simulator session (the tlon-workflow loop), run every `agent-device`
command below as `node <worktree>/.agents/skills/tlon-workflow/eas-device.mjs
<command>`, without `--session`, and add `--hide-touches` to `record start`.
The device is remote: `xcrun simctl` and `adb` do not reach it, so each
paragraph that uses them says what to do instead. Hosted QA drives a simulator
on its own Mac worker and uses the local commands as written.
...
Attachments: the emulator has no photos (`adb push` one, then `am broadcast -a android.intent.action.MEDIA_SCANNER_SCAN_FILE -d file://<path>`); on iOS grant photo access before opening the app (`xcrun simctl privacy <udid> grant photos <bundle id>`), because `alert dismiss` on the permission prompt denies it and the recovery relaunches the app; the composer's `+` has no label, so it takes coordinates. On an EAS device there is no way to push a file, so the library holds only what the device image ships with, and the grant is `eas-device.mjs settings permission grant photos` once the app is open; neither is checked yet, so say which you relied on.

Backgrounding, when the ticket or the variation calls for it: `agent-device home --session <name>` on either device; back with `xcrun simctl launch <udid> <bundle id>` on iOS and `adb -s <serial> shell am start -n <bundle id>/io.tlon.landscape.MainActivity` on Android. On an EAS device, `eas-device.mjs home` and back with `eas-device.mjs open <bundle id>`. A system activity over the app is `adb shell am start -a android.settings.INPUT_METHOD_SETTINGS`, which has no EAS equivalent.
