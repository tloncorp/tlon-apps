...
-   With `DEFAULT_SHIP_LOGIN_URL` and `DEFAULT_SHIP_LOGIN_ACCESS_CODE` set there too (the same pair `apps/tlon-mobile/.env.local` uses), the dev server answers the ship's login page with the access code already filled in and submits it, so a browser on this machine signs itself in; `DEFAULT_SHIP_LOGIN_URL` also stands in for `VITE_SHIP_URL`
