...
test('assessment rejects a product-changing QA ref before contacting EAS', () => {
  ...
  try {
    ...
    git('commit', '-am', 'wrong overlay');
    const overlay = git('rev-parse', 'HEAD');
    ...
    assert.equal(existsSync(path.join(dir, 'eas-contacted')), false);
  } finally {
