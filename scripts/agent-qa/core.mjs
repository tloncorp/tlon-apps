...
export function verifyContext(env, harnessSha) {
  ...
  if (env.QA_MODE === 'pull_request') {
    if (!pr?.head?.sha || pr.head.sha !== env.QA_BUILD_SHA || harnessSha !== pr.head.sha)
      throw new Error('PR head, EAS build commit, and checked-out source do not match');
  ...
  return {
    mode: env.QA_MODE === 'pull_request' ? 'PR verification' : 'Harness validation only',
    buildId: env.QA_BUILD_ID, buildSha: env.QA_BUILD_SHA, harnessSha,
    appId: env.QA_APP_ID, testShip: env.QA_TEST_SHIP, pr,
  };
}
...
export function commandFor(action) {
  ...
  switch (action.kind) {
    ...
    case 'press': return ['press', target(action.target)];
    case 'fill':
...
export function verifyReport(report, evidence) {
  if (!statuses.includes(report.status) || typeof report.summary !== 'string' ||
      !Array.isArray(report.checks) || !report.checks.length)
    throw new Error('Report must include a status, summary, and explicit checks');
  for (const check of report.checks) {
    if (!statuses.includes(check.status) || !check.expected || !check.observed ||
        !Array.isArray(check.evidence) || check.evidence.some((id) => !evidence.has(id)))
      throw new Error('Each check needs expected/observed behavior and valid evidence IDs');
    if (check.status !== 'blocked' && !check.evidence.length)
      throw new Error('Passed and failed checks require captured evidence');
  }
  const derived = report.checks.some((check) => check.status === 'failed') ? 'failed'
    : report.checks.some((check) => check.status === 'blocked') ? 'blocked' : 'passed';
  if (report.status === 'passed' && derived !== 'passed')
    throw new Error('Incomplete or failed checks cannot produce a pass');
  if (report.status === 'passed' && ![...evidence.values()].some((item) => item.screenshot))
...
export function renderReport(context, report, usage) {
  const clean = (value) => String(value).replace(/@/g, '@\u200b').slice(0, 3000);
  return [
    `**${context.mode}: ${report.status}**`, '', clean(report.summary), '',
