...
const { parseFen, applyMove, getRandomMove } = require('./chess-utils.cjs');

const BASE = 'malmur-halmex.tlon.network';
const PASSWORD = 'tastel-rospel-forryn-famnep';
  ...
  for (const [id, post] of newPosts) {
    ...
    try {
      ...
      if (agentMove) {
        ...
        console.log(`[move] agent responds: ${agentMove}`);
      }
