...
## EAS build cache findings (verified working)

End-to-end validation 2026-05-19 from this branch (`janic/build-perf-prototype`):

**Cache hit collapses native build from ~5–6 min to ~25s.** Measured via direct `EXPO_DEBUG=1 npx expo run:ios --no-bundler` from main repo with the cache pre-primed:

```
Searching builds with matching fingerprint on EAS servers       — 0s
Successfully downloaded cached build  (31.9 MB)                 — 18s
Installing on iPhone 16 Pro                                     — 23s
Opening on iPhone 16 Pro (io.tlon.groups)                       — 25s
```

vs. cold native_build from the harness: **288–369s.** So **~93% reduction** on the cache-hit path. No xcodebuild, no pod compile, no codegen, no anything — just download + simctl install + launch.

### Fingerprint stability gotcha (important)

The cache key is the EAS-computed fingerprint, generated by `eas-cli fingerprint:generate` (NOT `@expo/fingerprint` directly — the eas-build-cache-provider implements its own `calculateFingerprintHash` that shells out to eas-cli). **The fingerprint algorithm changed between eas-cli 18.7.0 and 19.0.1.** Old uploads at one version's hash are invisible to lookups from the other version.

Implications:
- **Pin eas-cli version across team + CI** if you want a shared cache to work consistently. Either via `eas-cli` as a workspace devDep, or via documented "install eas-cli@X" in onboarding. Floating versions mean floating cache keys.
- **After an upgrade, the cache needs re-priming**: one full build per platform to populate the cache at the new hash. Subsequent runs hit.
- `apps/tlon-mobile` content (tracked files + `app.config.ts` + Podfile + native dirs + relevant `node_modules` Expo config plugins) is what feeds the fingerprint. We're not currently using `sourceSkips` — could trim further if specific fields cause spurious churn.

### Provider mechanics (verified)

Reading [eas-build-cache-provider/build/index.js](../node_modules/eas-build-cache-provider/build/index.js):

- **Lookup**: `npx eas-cli build:download --platform=<p> --fingerprint=<h> --non-interactive --dev-client --json` (the `--dev-client` flag is set based on `expo-dev-client` being a direct dep + no `variant=release`/`configuration=Release` override — true for `expo run:ios` from this project).
- **Upload**: `npx eas-cli upload --platform=<p> --fingerprint=<h> --build-path=<path> --non-interactive --json` after a local build completes.
- **Fingerprint compute**: `npx eas-cli fingerprint:generate --platform=<p> --json --non-interactive`. NOT the local `@expo/fingerprint` (those give different hashes — eas-cli has its own logic for what to hash + uploads sources to EAS for `eas fingerprint:compare`).
- Errors are caught and logged via `Log.debug` (gated by `EXPO_DEBUG=1`), then the provider returns `null` so Expo CLI silently falls back to local build. Hard to debug without explicitly enabling debug logging.

### `EXPO_NO_CACHE=1` semantics

Confirmed: setting this env var still allows the provider to log "Searching builds with matching fingerprint…" but causes the actual `build:download` shell-out to bypass the cache lookup result, AND skips the post-build upload. Use it for "true cold" benchmarks that don't pollute the EAS cache.

### Recommended next moves on the EAS side

1. **Pin eas-cli.** Add `eas-cli` to mobile app's devDependencies at a specific version. Or document in CLAUDE.md a known-good version.
2. **Run a fresh cold+warm pair on the harness** with everyone on eas-cli 19.0.1 to get true post-upgrade numbers. The cold→warm delta should now include the ~5–6 min native_build collapse, not just the local-cache savings we measured before.
3. **Investigate the intermittent warm-miss** observed pre-upgrade (warm v1, v2 both missed despite identical fingerprints visible on EAS). If it recurs post-upgrade, file an issue against `eas-build-cache-provider` with `EXPO_DEBUG=1` logs.
