...
- Harness's `expo run:ios --no-bundler` computed fingerprint `65bc6902…` at lookup time (pre-build state).
...
The cache key is the EAS-computed fingerprint, generated by `eas-cli fingerprint:generate` (NOT `@expo/fingerprint` directly — the eas-build-cache-provider implements its own `calculateFingerprintHash` that shells out to eas-cli). **The fingerprint algorithm also changed between eas-cli 18.7.0 and 19.0.1.** Old uploads at one version's hash are invisible to lookups from the other version. So in addition to the `.fingerprintignore` fix, the team needs a consistent eas-cli version.
