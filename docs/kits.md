...
- **Install** binds a package to a place. For `%group`-scoped kits, install instantiates: it creates the group, creates each abstract place (`chat`/`gallery` as channels via `%channels`; `notebook` as a %notes notebook via `%notes-action-1` `%create-group-notebook`), writes the per-group config JSON into the group's `blob` field (via `%group-action-5`), and records an `$install` ledger entry. v1 is instantiate-only — every place a kit touches is a place it created.
- **Notebook places resolve at runtime.** %notes slugifies and assigns the notebook's flag itself and self-registers the channel with the group, so the installer cannot know the nest. Notebook places are therefore absent from the ledger's and blob's places maps; executors resolve them from the group's channel state (nests with the `notes/` kind prefix, matched by the manifest place title). The %notes poke is fire-and-forget: the request-id is synthesized from entropy and the response fact is ignored.
...
- `%install [id name meta]` — instantiate (see above). Asserts the kit exists, is `%group`-scoped, and no install exists for `[our name]`. Emits, in order: group create (`%group-command`), one create per place (`%channel-action-2` for chat/gallery, `%notes-action-1` for notebooks), the blob write, and an `%installed` fact.
