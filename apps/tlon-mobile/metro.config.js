...
//
// Wired unconditionally: behind an opt-in flag the store is off for every
// checkout that does not set the flag, which costs exactly what having no store
// costs. sharedCacheStores also registers the directory with rn-iso, so
// `rn-iso gc` can trim it -- Metro's FileStore has no eviction of its own.
const rnVersion = require('react-native/package.json').version;
const expoVersion = require('expo/package.json').version;
const cacheStores = sharedCacheStores(
  `tlon-mobile-rn-${rnVersion}-expo-${expoVersion}`
);
