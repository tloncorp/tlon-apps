## Summary

<!-- Briefly describe what this pull request does in 1-3 sentences. -->

## Changes

<!-- Outline specific changes made in this PR. -->

## How did I test?

<!-- Describe your testing process. -->

## Risks and impact

- Safe to rollback without consulting PR author? (Yes | No)
- [ ] Desk compatibility: every added or changed request in the **Desk
      requests** comment is one the N-1 desk serves, or is flagged in review —
      nothing there fails CI (`docs/tlon-apps/desk-compatibility.md`).
- Affects important code area:
  - [ ] Onboarding
  - [ ] State / providers
  - [ ] Message sync
  - [ ] Channel display
  - [ ] Notifications
  - [ ] Other:

## Rollback plan

<!-- Describe the steps to revert these changes if needed. -->

## Screenshots / videos

<!-- Attach any relevant media. -->
