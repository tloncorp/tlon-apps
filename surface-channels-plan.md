...
- **`surface preview <bundle> <spec>`** — renders the app **the way production does** and screenshots it, so the authoring bot can look at its own output. Assembles through the *same* document function `packages/app` uses (`@tloncorp/surface-shell/sandbox`), with the real shell artifact, the real CSP and the real bridge protocol — preview equals production **by construction**, not by resemblance, and a test reads the app's container component and fails if it ever stops using the shared assembler.
