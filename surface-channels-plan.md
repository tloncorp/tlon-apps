...
**Refusals: skip or abort** (amended after ratification — the original rule was skip-and-continue for every per-op refusal; a second amendment moved shape refusals from the skip side to the abort side). An op is refused either because it is not a well-formed op or because state cannot take the write, and the reducer does a different thing about each:

- **The op is malformed** — a bad or over-long pointer, a forbidden segment, `$actor` in a host op, a value that is not surface JSON. **That op alone is void and the entry continues in order.** Nothing was ever asked of state, so the refusal is identical against every state, on every client, at every fold position; the author meets it on the first fold, and the rest of a mostly correct entry is still worth applying.
- **State cannot take the write** — its shape has no such path (a scalar or array where an object must be traversed, an `append` onto a non-array), or the result would be more than state may hold (the reduced-state size cap, the JSON depth cap). **The op is refused and so is every op after it in that entry.** The op is well formed and is exactly what the author meant; what refused it is the state it met.

**The criterion is which of the two was wrong — the op, or the state it was applied to** — and that is what places a refusal kind added later, rather than the list above. It is deliberately not "could the author have seen it coming": the depth cap is computable from the op alone and would sort with the malformed ops on that test. It aborts anyway, because what it *means* is "state cannot hold this", and a `del` after a depth-refused `set` destroys data exactly as it does after a size-refused one.

The reason for the split is what continuing costs. Under skip-and-continue the state after an entry is an arbitrary **subsequence** of its ops, and a subsequence can contain a destructive op without the op it depended on. The host-is-the-clock rollover is exactly that shape — `set /history/<date>` then `del /today` — and the archiving `set` is the one that fails: near the cap because it grows state while the shrinking `del` still applies, and at any size if `/history` holds something other than an object. Either way the day ends up neither archived nor present. Aborting makes the state after an entry a **prefix** of its ops, which is the strongest property available without a transaction log, and it is precisely what makes "archive, then clear" safe: the clear is unreachable unless the archive landed.

Deterministic and convergent, which is the condition for changing the one component every client runs identically. State is a pure function of the post log; all three refusals on the abort side are pure functions of state and the op — a shape mismatch reads the folded state, which is itself a pure function of the log, so it is no less determined than a cap. Every client therefore aborts at the same op of the same entry. Nothing in the decision reads a clock, an allocator, or any other client-local quantity.

An aborted entry still counts as folded and still advances the coverage watermark — it was processed to a deterministic conclusion. `stateFull` continues to mean the size cap specifically, and stays narrow on purpose: it is the one refusal a host repairs by snapshotting and pruning, which is what "dashboard full" asks for, while pruning neither makes a path shallower nor turns a scalar into an object. The reduction reports `abortedEventCount` for all three, which tells a host that its last entry landed only in part and must be re-posted once the refusal is dealt with. Note that a host can always still shrink state: a `del`-only entry never grows state, so it is never refused for size.
...
- **Commands:** `surface create` (clear error when the moon lacks the admin role); `surface templates list|show`; `surface lint <bundle> <spec>`; `surface publish <channel> --bundle --spec` (gate → upload to storage → sha256 → read-modify-write the description with revision bump → mirror post → **migration snapshot in the same command when `preserveState`**, keeping the pending window one command wide); `surface event` (host ops); `surface state` (hydrate + reduce via the shared reducer); `surface snapshot`; **`surface preview <bundle> <spec>`** (below); **`surface fork <source-channel> [--into <group>] [--regenerate]`** (below, Session 6).
...
- **No local stand-in for storage.** `surface publish` stops at `storage-unavailable` without an S3-compatible endpoint, so **the publish loop is not runnable against a fakeship** — the grant above is a hard dependency for development, not only for production. The seed sidesteps this for *rendering* with a localhost bundle server (`packages/shared/seed/bundleServer.ts`), exactly because provisioning a bucket is an out-of-repo human step; publishing has no equivalent, and a repo-owned dev-storage stub is the obvious fix.
