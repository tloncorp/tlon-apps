...
**Refusals: skip or abort** (amended after ratification — the original rule was skip-and-continue for every per-op refusal). The reducer refuses an op for one of two reasons, and does a different thing about each:

- **The op is wrong** — a malformed or over-long pointer, a forbidden segment, `$actor` in a host op, a value that is not surface JSON, or a write through a shape that has no such path (a scalar or array where an object must be traversed, an `append` onto a non-array). **That op alone is void and the entry continues in order.** These are author errors: they are the same on every client and at every fold position, the author sees them on the first fold, and skipping them lets the rest of a mostly correct entry apply.
- **A resource cap refuses it** — the reduced-state size cap, or the JSON depth cap. **The op is refused and so is every op after it in that entry.** The op is exactly what the author meant; the only reason it did not land is that state has grown into a cap, which is a condition of the environment rather than a defect in the entry.

The reason for the split is what continuing costs. Under skip-and-continue the state after an entry is an arbitrary **subsequence** of its ops, and a subsequence can contain a destructive op without the op it depended on. The host-is-the-clock rollover is exactly that shape — `set /history/<date>` then `del /today` — and near the cap the archiving `set` was refused while the shrinking `del` still applied, leaving the day neither archived nor present. Aborting makes the state after an entry a **prefix** of its ops, which is the strongest property available without a transaction log, and it is precisely what makes "archive, then clear" safe: the clear is unreachable unless the archive landed.

Deterministic and convergent, which is the condition for changing the one component every client runs identically. State is a pure function of the post log; both caps are pure functions of state and the op; so every client aborts at the same op of the same entry. Nothing in the decision reads a clock, an allocator, or any other client-local quantity.

An aborted entry still counts as folded and still advances the coverage watermark — it was processed to a deterministic conclusion. `stateFull` continues to mean the size cap specifically, because that is the refusal a host repairs by snapshotting and pruning; the reduction additionally reports `abortedEventCount`, which tells a host that its last entry landed only in part and must be re-posted once the cap is dealt with. Note that a host can always still shrink state: a `del`-only entry never grows state, so it is never refused.
