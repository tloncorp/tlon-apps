...
describe('createInflightCheckpoint', () => {
  it('upserts, removes, and survives reload', () => {
    ...
    expect(
      checkpoint
        ...
        .map((l) => l.lensId)
        .toSorted()
    ).toEqual([a.lensId, b.lensId].toSorted());
  ...
  it('evicts the oldest entry past maxInflight and warns about it', () => {
    const warnings: string[] = [];
    const checkpoint = createInflightCheckpoint({
      filePath: inflightCheckpointPath(filePath),
      maxInflight: 2,
    ...
    expect(warnings.join('\n')).toContain(a.lensId);
    expect(warnings.join('\n')).toContain('will not be recovered');
  });
});
...
describe('recoverInterruptedRuns', () => {
  ...
  it('keeps entries whose store save fails checkpointed for the next boot', () => {
    const store = createContextLensStore({ filePath });
    const failing = makeInflightLens({ messageId: 'fails' });
    const ok = makeInflightLens({ messageId: 'ok' });
    const checkpoint = createInflightCheckpoint({
      filePath: inflightCheckpointPath(filePath),
    });
    checkpoint.upsert(failing);
    checkpoint.upsert(ok);

    const flakyStore = {
      ...store,
      save: (lens: ContextLens) => {
        if (lens.lensId === failing.lensId) {
    ...
    expect(recovered).toBe(1);
    expect(store.get(ok.lensId)?.status).toBe('aborted');
    expect(store.get(failing.lensId)).toBeNull();
    // The failed entry survives for the next boot's recovery attempt.
    expect(checkpoint.list().map((l) => l.lensId)).toEqual([failing.lensId]);
  });
});
