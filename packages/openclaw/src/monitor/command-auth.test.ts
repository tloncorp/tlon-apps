...
import { checkOwner, resolveBridgeForCommand } from './command-auth.js';
import {
...
function makeBridge(ownerShip: string | null): ApprovalCommandBridge {
  return {
    ...
    setOwnerListenDisabled: async (_nest: string, disabled: boolean) =>
      !disabled,
...
describe('checkOwner', () => {
  ...
  it('rejects when senderId is missing', () => {
    const bridge = makeBridge('~zod');
    expect(checkOwner({}, bridge)).toEqual({
