...
function makeHarness(opts: HarnessOpts): Harness {
  ...
  const api = {
    scry: vi
      ...
      .mockImplementation(
        opts.scryImpl ??
          (async () => opts.scryResult ?? { all: { moltbot: { tlon: {} } } })
      ),
    poke: vi
  ...
  const setLocalPendingNudge =
    vi.fn<(accountId: string, nudge: PendingNudge) => void>();
  ...
  const deps: NudgeRunnerDeps = {
    ...
    getEffectiveOwnerShip: () =>
      opts.owner === undefined ? OWNER_SHIP : opts.owner,
...
describe('nudge-runner', () => {
  it('happy path: idle 8 days, fresh scry 0, sends stage 1', async () => {
    ...
    const poke = h.api.poke.mock.calls[0][0] as {
