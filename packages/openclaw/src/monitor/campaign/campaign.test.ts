...
describe('campaign evaluator', () => {
  it('uses consecutive hourly windows in the accelerated testing profile', () => {
    const config = {
      testing: {
        intervalMinutes: 60,
    ...
    expect(
      evaluateCampaign(
        state({
          timezone: undefined,
          sent: [{ step: 'useful-request', at: enrolledAt + 60 * MINUTE }],
        }),
        facts,
        enrolledAt + 120 * MINUTE,
        config
      )
    ).toEqual({ kind: 'send', step: 'recurring-help' });
  });
...
describe('campaign runner', () => {
  it('retains a fresh enrollment until the store recovers without resetting its server time', async () => {
    ...
    expect(memory.read().lastActivityAt).toBe(enrolledAt);
    await h.campaign.enroll({
      isFirstGroup: true,
      campaignVersion: 1,
...
describe('ticket conversation flow', () => {
  ...
  it('does not append a generic recurring pitch to a useful reply', async () => {
    const h = harness();
    await h.campaign.check();
    const context = await h.campaign.replyContext();
    expect(context).toContain('Do not append a generic recurring-task pitch');
    expect(context).not.toContain('Would this be useful every week?');
  });
...
it('withholds recurring setup while task facts are loading', async () => {
  const h = harness();
