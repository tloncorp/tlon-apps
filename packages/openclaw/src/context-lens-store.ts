  ...
  return {
    ...
    upsert: (lens) => {
      ...
      while (pending.size > maxInflight) {
        ...
        logger?.warn(
          `[tlon] Context lens in-flight checkpoint full (${maxInflight}); evicting ${oldest} — it will not be recovered if the gateway crashes`
        );
      }
      flush();
    },
 ...
 * whose terminal removal was lost to the crash). Checkpoint entries are
 * removed individually as they are handled; an entry whose store save fails
 * stays checkpointed so the next boot can retry it.
 */
...
export function initContextLensStore(api: {
  ...
  storeUnsubscribeSlot.set(
    subscribeToContextLensEvents((event) => {
      ...
      if (TERMINAL_STATUSES.has(lens.status)) {
        // Durable save first: if the save fails (or the process dies between
        // the two writes), the checkpoint entry survives and boot recovery
        // finalizes the run instead of losing it entirely.
        try {
          store.save(lens);
        } catch (error) {
          api.logger.warn(
