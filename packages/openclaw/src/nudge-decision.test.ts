...
describe('nudge-decision', () => {
  ...
  describe('inActiveHours', () => {
    ...
    it('returns false outside a forward window', () => {
      expect(inActiveHours(new Date('2026-04-21T22:00:00Z'), hours)).toBe(
        false
      );
    });
    ...
    it('includes the start boundary and excludes the end boundary', () => {
      ...
      expect(inActiveHours(new Date('2026-04-21T21:00:00Z'), hours)).toBe(
        false
      );
    });
    ...
    it('treats "00:00"–"24:00" as a full-day window', () => {
      ...
      expect(inActiveHours(new Date('2026-04-21T00:00:00Z'), fullDay)).toBe(
        true
      );
      expect(inActiveHours(new Date('2026-04-21T23:59:00Z'), fullDay)).toBe(
        true
      );
    });
    ...
    it('honors the configured timezone', () => {
      ...
      expect(inActiveHours(new Date('2026-04-21T12:00:00Z'), nyHours)).toBe(
        false
      );
      // 14:00 UTC is 10:00 in New York — inside the window.
      expect(inActiveHours(new Date('2026-04-21T14:00:00Z'), nyHours)).toBe(
        true
      );
    });
  });
  ...
  describe('resolveActiveHours', () => {
    ...
    it('field-wise overlay: timezone-only edit with the "user" keyword resolves via userTimezone', () => {
      const result = resolveActiveHours({ nudgeActiveHoursTimezone: 'user' }, {
        ...
        channels: {
          tlon: {
            nudgeActiveHours: { start: '08:00', end: '20:00', timezone: 'UTC' },
          },
        },
      } as never);
      expect(result).toEqual({
    ...
    it('field-wise overlay: single-bound settings edit overlays onto baseline', () => {
      const result = resolveActiveHours({ nudgeActiveHoursEnd: '22:00' }, {
        channels: {
          tlon: {
            nudgeActiveHours: { start: '09:00', end: '17:00', timezone: 'UTC' },
          },
        },
      } as never);
  ...
  describe('resolveLastOwnerInstant', () => {
    ...
    it('returns null when date string is unparseable', () => {
      expect(
        resolveLastOwnerInstant(null, { lastOwnerMessageDate: 'nope' })
      ).toBeNull();
    });
  });
