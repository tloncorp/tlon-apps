...
describe('connectivity', () => {
  ...
  test('scries contacts from bot ship', async () => {
    ...
    console.log(
      `✓ Got ${Array.isArray(contacts) ? contacts.length : 0} contacts from bot ship`
    );
  ...
  test('scries groups from bot ship', async () => {
    ...
    console.log(
      `✓ Bot has ${Array.isArray(groups) ? groups.length : 0} groups`
    );
  ...
  test('can read DM channel posts from test user', async () => {
    ...
    try {
      ...
      console.log(
        `✓ Got ${Array.isArray(posts) ? posts.length : 0} posts from DM channel`
      );
      if (Array.isArray(posts) && posts.length > 0) {
        console.log(
          `  Sample post:`,
          JSON.stringify(posts[0], null, 2).slice(0, 500)
        );
      }
