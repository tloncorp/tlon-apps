...
describe("messages", () => {
  ...
  function botReaction(post: PostLike | undefined, botShip: string): string | undefined {
  ...
  async function getBotNickname(): Promise<string> {
    const profile = await fixtures.botState.scry<Record<string, unknown>>("contacts", "/v1/self");
  ...
  describe("channel engagement", () => {
    test("engages on @ship mention", async () => {
      ...
      await fixtures.userState.sendPost({
        ...
        content: storyWithMention(fixtures.botShip, `[tlon-test:${key}] hello bot`),
      });
    ...
    test("engages on nickname mention", async () => {
      ...
      await fakeModel.script(key, [{ kind: "text", content: "Acked nickname" }]);
    ...
    test("engages on reply to bot's own channel post (thread participation)", async () => {
      ...
      await fakeModel.script(key, [{ kind: "text", content: "Acked thread reply" }]);
  ...
  describe("reactions", () => {
    test("adds then removes a reaction via the message tool", async () => {
      ...
      const parent = await findChannelPost(target, fixtures.userShip, parentToken, 15_000);
      ...
      if (!addResp.success) {throw new Error(addResp.error ?? "add prompt failed");}
      ...
      await waitFor(async () => {
        ...
        return typeof value === "string" && value.includes(emoji) ? true : undefined;
      }, 30_000);
      ...
      if (!rmResp.success) {throw new Error(rmResp.error ?? "remove prompt failed");}
      ...
      const removed = await waitFor(async () => {
        ...
        return botReaction(updated, fixtures.botShip) == null ? true : undefined;
      }, 30_000);
