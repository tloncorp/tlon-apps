...
# The image defaults to the plugin's supported 7.1 runtime. Future versions
# must carry an explicit prewarm expectation so a missing core marker never
# silently becomes smoke.
OPENCLAW_CORE_VERSION="${OPENCLAW_CORE_VERSION:-2026.7.1-2}"
case "$OPENCLAW_CORE_VERSION" in
  2026.5.28) derived_expect_prewarm=0 ;;
  2026.6.11|2026.7.1|2026.7.1-2) derived_expect_prewarm=1 ;;
  *)
