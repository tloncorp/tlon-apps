...
const compiledSchemaPath = path.join(
  repoRoot,
  'dist',
  'src',
  'config-schema.js'
...
const { tlonChannelConfigSchema } = await import(
  pathToFileURL(compiledSchemaPath).href
);
...
manifest.channelConfigs.tlon = {
  ...
  ...(tlonChannelConfigSchema.uiHints
    ? { uiHints: tlonChannelConfigSchema.uiHints }
    : {}),
};
