...
After the fake ships become ready, the runner assembles and applies the checkout's `%groups` desk. `TLON_BOT_E2E_DESK_SHIPS` selects a comma-separated ship set and defaults to `~zod,~ten`. This requires `peru` and `rsync`, as described in the root desk-dependency documentation.
