...
class TlonAdapter(BasePlatformAdapter):
    ...
    async def send(
        ...
        with cli_context("delivery", conversation=chat_id):
            # parentAuthor: honor what Hermes passes. For one-to-one DMs the
            # writ id itself is author-prefixed, so use that exact prefix
            # when core did not supply metadata (not the partner default the
            # CLI would otherwise assume).
