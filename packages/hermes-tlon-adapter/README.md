...
**Reply-visibility parity (TLON-6317).** OpenClaw's plugin has to pin `sourceReplyDeliveryMode: 'automatic'` on every turn because OpenClaw core can resolve an assistant final to `message_tool_only` — kept private — via `messages.visibleReplies`, `messages.groupChat.visibleReplies`, or a harness delivery default (`packages/openclaw/src/monitor/source-reply-delivery.ts`). Hermes core, as pinned here (tag `v2026.6.19`, commit `2bd1977d8`), has no reply-visibility policy at any layer: no config axis, no delivery-mode field on `MessageEvent`, no visibility parameter in the `send()` contract. Core hands every non-empty reactive final to `send()` (`gateway/platforms/base.py:4335`); the only core-side intentional suppression is the model-emitted whole-string `NO_REPLY`/`[SILENT]` silence sentinel, which is model behavior, not operator config. Scope fences: attachment-only output is the separate TLON-6318 outbound-media dimension; the directive-only sanitization inside this adapter's `send()`
is deliberate adapter behavior, not a visibility policy; and non-reactive sends (nudges, approval cards, migration notices, cron) go out via direct CLI sends and likewise consult no policy. This adapter therefore deliberately contains no visibility handling.
