...
-   **Inbound cite placeholders** render as the constant `[quoted message]` in the message body itself (OpenClaw uses a nest-bearing placeholder); constant text there avoids sender-controlled placeholder content by design, because the body text is what the mention matcher reads. Cite enrichment, which runs strictly _after_ the mention decision, does prepend richer lines: resolved channel quotes and `> [ref: group ~host/slug]` pointers whose flag passed a fullmatch regex — still no free-form sender text in a placeholder.
