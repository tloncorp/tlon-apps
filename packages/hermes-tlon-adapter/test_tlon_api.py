...
class TlonCLITests(unittest.TestCase):
    ...
    def test_message_text_that_looks_like_the_flag_is_delivered(self):
        # These bots answer questions about this CLI, so `--bot` is plausible
        # message content. Scanning the whole command would read the message as
        # a caller-supplied flag: the send would go out undecorated and the new
        # CLI would then treat the text as its own message boundary.
        for message in ("--bot", "--bot=Botly"):
            with self.subTest(message=message):
                ...
                asyncio.run(
                    cli.run_command(
                        ("posts", "send", "chat/~zod/general", message)
                    )
                )
                ...
                # The message survives, and the flag is still appended.
                self.assertEqual(
                    sends,
                    [
                        (
    ...
    def test_a_reply_message_that_looks_like_the_flag_is_delivered(self):
        # The reply forms carry an extra positional, so the option region
        # starts one argument later; an off-by-one here would eat the message.
        calls = []
        ...
        asyncio.run(
            cli.run_command(
                ("posts", "reply", "chat/~zod/general", "170.141", "--bot")
            )
        )
