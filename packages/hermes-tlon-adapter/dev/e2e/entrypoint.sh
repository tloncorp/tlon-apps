...
mkdir -p "$HERMES_HOME/plugins/platforms" "$HERMES_HOME/logs" "$HERMES_HOME/memories" /workspace/hermes-dev/bin
rm -f "$HERMES_HOME/config.yaml" "$HERMES_HOME/SOUL.md" "$HERMES_HOME/.hermes.md" "$HERMES_HOME/memories/USER.md"
...
def write_managed_prompt(target, rel):
...
write_managed_prompt(home / "SOUL.md", "hermes/SOUL.md")
write_managed_prompt(home / ".hermes.md", "hermes/.hermes.md")
write_managed_prompt(home / "memories" / "USER.md", "hermes/USER.md")
PY

echo "==> Writing Hermes E2E config in $HERMES_HOME/config.yaml"
"$HERMES_VENV/bin/python" "$TLON_ADAPTER_DIR/dev/e2e/write_config.py"
...
echo "==> Hermes E2E setup complete"
echo "==> Starting Hermes gateway"
read -r -a GATEWAY_ARGS <<< "${HERMES_GATEWAY_ARGS:---replace -v}"
