...
function isMergeable(x: InlineKey): x is (typeof MERGEABLE_KEYS)[number] {
  return MERGEABLE_KEYS.includes(x as (typeof MERGEABLE_KEYS)[number]);
}

/**
 * Normalize an array of Inline elements by merging adjacent elements with the same key.
 * For example, [{italics: ["a"]}, {italics: ["b"]}] becomes [{italics: ["a", "b"]}].
 * This fixes issues where Tiptap represents styled text with links as separate styled segments.
 */
export function normalizeInline(inline: Inline[]): Inline[] {
  return inline.reduce<Inline[]>((acc, val) => {
    if (acc.length === 0) {
      return [val];
    }
    ...
    if (typeof last !== 'object' || typeof val !== 'object') {
      return [...acc, val];
    }
    const lastKey = Object.keys(last)[0] as InlineKey;
    const currKey = Object.keys(val)[0] as InlineKey;
    if (isMergeable(lastKey) && currKey === lastKey) {
      // Use any to bypass strict typing - these are known to be objects with array values
      const lastObj = last as unknown as Record<string, Inline[]>;
      const valObj = val as unknown as Record<string, Inline[]>;
      const end: Inline = {
        [lastKey]: [...lastObj[lastKey], ...valObj[currKey]],
