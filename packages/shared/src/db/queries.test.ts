...
test('reconcileJoinedGroupChannels: two-way reconcile across all channel kinds', async () => {
  setScryOutputs([initResponse]);
  await syncInitData();
  ...
  const joinedBefore = groupBefore!.channels.filter(
    (c) => c.currentUserIsMember === true
  );
  expect(joinedBefore.length).toBeGreaterThan(1);

  // Reconcile to a single joined channel: every other group channel — of any
  // kind — must be marked left, and the one in the list kept joined.
  const keepJoined = joinedBefore[0].id;
  const nowLeft = joinedBefore.slice(1).map((c) => c.id);

  await queries.reconcileJoinedGroupChannels({
    joinedChannelIds: [keepJoined],
  });
  ...
  expect(
    groupAfter!.channels.find((c) => c.id === keepJoined)?.currentUserIsMember
  ).toBe(true);
  for (const channelId of nowLeft) {
    expect(
      groupAfter!.channels.find((c) => c.id === channelId)?.currentUserIsMember,
      `channel ${channelId} should now be left`
    ).toBe(false);
  }
});
