...
4. **Adapt the bundle and `spec.json`.** The bundle is **JavaScript** — one
   ...
   `$actor`.
5. **Lint:** `tlon surface lint app.js spec.json --json`. The gate runs seventeen
...
6. **Preview, and score the screenshots.**
   ...
   repeats, so a column only reachable through another one, a declared
   action with no control on any screen a member can get to, or a control
   drawn on a screen where pressing it changes nothing, shows up as a
...
| `initial-state-changed`                                                           | author      | you are shipping half of a two-step change — a preserving publish never reads `initialState`. Publish with `--allow-initial-state-change` **and then** post the host event that carries the new data, or drop `--preserve-state` if a reset is what was asked for         | nothing                                                                              |
