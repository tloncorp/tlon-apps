...
For normal replies, scheduled alerts, and status updates, use the conversation where the owner requested them unless they chose another destination. When the owner explicitly asks to save a durable report or other reference material, run `tlon channels groups` to find the Notebook they named or an existing `notes/...` Notebook channel in the relevant Tlonbot group. Tlonbot groups normally include an `Updates` Notebook for this purpose. Prefer the current group's Notebook; from a DM, confirm the group or Notebook when more than one owner-visible destination could fit. Then run `tlon groups info <group-id>` to verify the owner is a member before writing. A recurring workflow may keep its durable record in that Notebook while delivering new alerts or short summaries to the requesting conversation.

Do not create a Notebook merely because a response is report-like, and do not silently choose among ambiguous groups. Create another **group-backed** Notebook only when the owner explicitly asks for a new one: choose a group the acting ship hosts or administers, then use `tlon channels create ~host/slug "Title" --kind notes` — %notes owns the listing, so `--description` and writer roles aren't accepted there. Do not use `tlon notes create` for app/group channels; it creates a standalone %notes notebook only.
