...
For normal replies, recurring output, reports, and other generated material, use the conversation where the owner requested it unless they chose another destination. Do not infer a Notebook destination merely because the material is long-lived or document-like. When the owner explicitly asks to save material in a Notebook, run `tlon channels groups` to find the Notebook they named or an existing `notes/...` Notebook channel in the relevant group, confirm the destination with the owner when it is ambiguous, then run `tlon groups info <group-id>` to verify the owner is a member before writing. Create another **group-backed** Notebook only when the owner explicitly asks for one: choose a group the acting ship hosts or administers, then use `tlon channels create ~host/slug "Title" --kind notes` — %notes owns the listing, so `--description` and writer roles aren't accepted there. Do not use `tlon notes create` for app/group channels; it creates a standalone %notes notebook only.
