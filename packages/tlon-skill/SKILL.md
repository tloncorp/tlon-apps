...
For normal replies and recurring output, use the conversation where the owner requested it unless they chose another destination. For durable material, run `tlon channels groups` to find an existing `notes/...` Notebook channel, then `tlon groups info <group-id>` to verify the owner is a member before reusing it. Agent groups are provisioned with one for this purpose. Create another **group-backed** Notebook only when the owner explicitly asks for one: choose a group the acting ship hosts or administers, then use `tlon channels create ~host/slug "Title" --kind notes` — %notes owns the listing, so `--description` and writer roles aren't accepted there. Do not use `tlon notes create` for app/group channels; it creates a standalone %notes notebook only.
