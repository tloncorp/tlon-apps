 ...
 * when that is the *only* binding of the name there and nothing assigns to it.
 *
 * This is the whole of the reader's local-variable reasoning, and it exists
 * for one spelling that is everywhere in this client:
 *
 *     const path = `/v3/ui/groups/${groupId}`;
 *     await scry({ app: 'groups', path });
 *
 * `getGroup` is one of the two call sites that broke build 440, so a reader
 * that could not follow that line would not catch the incident it exists to
...
/** `(a ? b : c)` is the same expression as `a ? b : c`. */
...
function literalValues(
  ...
  const resolve = (raw: ts.Expression): Val<string | null>[] => {
  ...
  if (prop === null || prop === 'shorthand') return [{ value: null }];
  return resolve(prop);
}

/** Every path a property can take, resolved down to its literal prefix. */
function pathValues(ctx: Ctx, raw: ts.Expression): Val<PathPattern>[] {
  const expr = unwrap(raw);
  ...
  if (ts.isConditionalExpression(expr)) {
    return branches(ctx, expr, (e) => pathValues(ctx, e));
  }
...
function expandHelper(ctx: Ctx, name: string, depth = 0): PokeParams[] {
  ...
  /**
   * Each return with the condition of the `if` it sits inside, which is how
   * `activityAction` gates `activity-action-2` behind
   * `getActivitySupportsNotes()`. Only the condition written around the return
   * counts: a return is never assumed to negate a condition for its siblings.
   */
  ...
  const walkStatements = (list: readonly ts.Statement[], guard?: string) => {
    for (const stmt of list) {
      if (ts.isReturnStatement(stmt)) {
        if (stmt.expression) returns.push([stmt.expression, guard]);
      } else if (ts.isIfStatement(stmt)) {
        const cond = textOf(ctx, stmt.expression);
        walkStatements(
          statementsOf(stmt.thenStatement),
          bothGuards(guard, cond)
        );
...
function readEndpoint(
  ...
  if (ts.isObjectLiteralExpression(arg)) {
    return readEndpointObject(ctx, node, arg, surface);
  }
  // An endpoint built somewhere this reader does not follow is still a branch
  // that reaches the ship, so it is recorded rather than dropped.
  push(ctx, node, {
    surface,
              ...
              value: {
                ...
                text:
                  prop === null
                    ? '<no path>'
                    : prop === 'shorthand'
                      ? 'path (shorthand, unresolved)'
                      : `${textOf(ctx, prop)} (unresolved)`,
              },
            },
          ]
        : pathValues(ctx, prop);
