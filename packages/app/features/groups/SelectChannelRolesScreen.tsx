...
import { RadioControl } from '../../ui/components/Form';
...
export function SelectChannelRolesScreen() {
    const navigation =
        useNavigation<
            NativeStackNavigationProp<GroupSettingsStackParamList, 'SelectChannelRoles'>
        >();
    const route =
        useRoute<RouteProp<GroupSettingsStackParamList, 'SelectChannelRoles'>>();
    const insets = useSafeAreaInsets();

    const { groupId, selectedRoleIds: initialRoleIds, returnScreen, returnParams } = route.params;
    ...
    const allRoles = useMemo(
        () => groupRolesToOptions(group?.roles ?? []).filter((role) => role.value !== 'admin'),
        [group?.roles]
    );
