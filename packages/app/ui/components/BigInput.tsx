...
import { LiveMarkdownInput } from './MessageInput/LiveMarkdownInput';
...
function useMarkdownMode({
  ...
  liveMarkdownInputEnabled,
  editorRef,
  showToast,
}: {
  editingPost?: db.Post;
  markdownNotebooksEnabled: boolean;
  liveMarkdownInputEnabled: boolean;
  ...
  // or if liveMarkdownInput is enabled (replaces the webview editor entirely)
  const [isMarkdownMode, setIsMarkdownMode] = useState(
    liveMarkdownInputEnabled || (markdownNotebooksEnabled && !editingPost)
  );
...
export function BigInput({
  ...
  const [liveMarkdownInputEnabled] = useFeatureFlag('liveMarkdownInput');
  ...
  const handlePasteImages = useCallback(
    async (images: PastedImage[]) => {
      ...
      try {
        // Build an ImagePickerAsset-compatible object from the pasted image
  ...
  // capabilities (Link, Undo, Redo, Indent/Outdent), add inline image button
  const enrichedToolbarItems = useMemo((): ToolbarItem[] => {
    ...
    // Insert after Heading button (index 3 after filtering)
        ...
        <View flex={1}>
          ...
          >
            {isMarkdownMode
              ? liveMarkdownInputEnabled
                ? '[LiveMarkdownInput]'
                : '[MarkdownEditor (plain)]'
              : enrichedInputEnabled
                ? '[EnrichedTextInput (native rich text)]'
                : '[MessageInput (webview)]'}
          </Text>
          {isMarkdownMode ? (
            liveMarkdownInputEnabled ? (
              <LiveMarkdownInput
