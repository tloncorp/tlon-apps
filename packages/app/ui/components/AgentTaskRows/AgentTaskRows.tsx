...
export type AgentTaskRowsProps = {
  ...
  /**
   * Called when a failed row's pill is pressed. Omit it and the failed state
   * renders without a retry affordance, rather than showing one that does
   * nothing.
   */
  onRetry?: (row: AgentTaskRow) => void;
  testID?: string;
};
...
function StatusPill({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel="Retry this step"
    >
      {pill}
    </Pressable>
  );
        ...
        >
          ...
          >
            <XStack minHeight={44} alignItems="center" gap="$m">
              ...
              <StatusPill status={row.status} onRetry={onRetry} />
              {row.meta ? (
                <SizableText
                  size="$xs"
                  color="$tertiaryText"
  ...
  const content = rows.map((row, index) => {
    ...
    return (
      <TaskRow
        ...
        onRetry={onRetry ? () => onRetry(row) : undefined}
      />
    );
  });
