 ...
 * concrete channels. The implementation lives here — in the one shared,
 * published package both readers depend on — so the client and the OpenClaw
 * harness read the same payload the same way; they previously had separate
 * implementations that disagreed about what a malformed payload means.
 * (@tloncorp/tlon-kits is monorepo-only and deliberately unpublished, which
 * is why it cannot host code the published packages need at runtime.)
 *
 ...
 * group's config.
 *
 * Stick to the zod 3 / zod 4 common subset here. This package pins zod 3
 * while the harness packages are on zod 4, and the web bundler collapses
 * them to a single copy, so a version-only API (v4's `z.looseObject`, say)
 * throws at module-eval time in the browser and takes the whole app down
 * with it.
 */
import { z } from 'zod';
