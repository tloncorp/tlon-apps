...
export const AgentProvisionActionContextSchema = z.object({
  ...
  /**
   * Set only when the owner explicitly names a different timezone.
   */
  timezoneOverride: agentProtocolString(
    AGENT_PROTOCOL_LIMITS.timezoneLength
  ).optional(),
  /** Fallback timezone captured from the onboarding interview context. */
  interviewTimezone: agentProtocolString(
    AGENT_PROTOCOL_LIMITS.timezoneLength
  ).optional(),
});
